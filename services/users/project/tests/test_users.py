...
class TestUserService(BaseTestCase):
    ...
    def test_add_user(self):
        ...
        with self.client:
            ...
            self.assertIn('yersoncardenas@upeu.edu.pe ha sido agregado exitosamente', data['message'])
            self.assertIn('success', data['status'])
    ...
    def test_all_users(self):
        ...
        with self.client:
            ...
            self.assertIn('yersoncardenas@upeu.edu.pe', data['data']['users'][0]['email'])
            self.assertIn('jerson',data['data']['users'][1]['username'])
            self.assertIn('cardenas.x7@gmail.com',data['data']['users'][1]['email'])
            self.assertIn('success',data['status'])

    def test_main_no_users(self):
        """Asegurando que la ruta principar funcione correctamente cuando no hay usuarios añadidos a la base de datos"""
    ...
    def test_main_with_users(self):
        """Asegurando de que todos los usuarios se funcionen correctamente cuando un usuario es correctamente agregado a la base de datos."""
        add_user('cardenas','yersoncardenas@upeu.edu.pe')
        add_user('jerson','cardenas.x7@gmail.com')
        with self.client:
            response=self.client.get('/')
    ...
    def test_main_add_user(self):
        """Asegurando que un nuevo usuario pueda ser agregada a la db mediante un POST request."""
        with self.client:
            response = self.client.post(
                '/',
                data=dict(username='cardenas', email='yersoncardenas@upeu.edu.pe'),
